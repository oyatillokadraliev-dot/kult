...
const SCHEDULE={
  mon:[{time:'09:00',name:'TRX / Функциональный тренинг',trainer:'Зося',key:'trx'},{time:'11:00',name:'TRX / Функциональный тренинг',trainer:'Зося',key:'trx'},{time:'17:00',name:'Силовая',trainer:'Ксения',key:'silovaya'},{time:'18:00',name:'TRX / Функциональный тренинг',trainer:'Ксения',key:'trx'},{time:'19:00',name:'Crossfit',trainer:'Ксения',key:'crossfit'}],
...
function initSiteImages(){
  // Все пути к файлам — относительно папки site/
  // Чтобы добавить фото тренера: положи файл в images/trainers/имя.jpg
  const BASE = '';  // пустой путь — файлы рядом с index.html

  // Логотип в навбаре
  const navLogo=document.getElementById('nav-logo-img');
  if(navLogo) navLogo.src = BASE + 'images/logo-dark.png';

  // Hero фон — для мобильных и десктопа разные файлы
  const heroBg=document.getElementById('hero-bg-photo');
  const isMobile=window.innerWidth<=680;
  if(heroBg){
    const src = isMobile
      ? BASE + 'images/hero-mobile.jpg'
      : BASE + 'images/hero-collage.jpg';
  ...
  // Фон шапки ЛК и кабинета тренера
  const equipSrc = BASE + 'images/equipment.jpg';
  const lkBg=document.getElementById('lk-bg-photo');
  if(lkBg){
    lkBg.style.backgroundImage=`url('${equipSrc}')`;
    requestAnimationFrame(()=>lkBg.classList.add('loaded'));
  }
  const tcBg=document.getElementById('tc-bg-photo');
  if(tcBg){
    tcBg.style.backgroundImage=`url('${equipSrc}')`;
